...
string exec(const char* cmd) 
{
    ...
    // unique_ptr<FILE, decltype(&pclose)> pipe(popen(cmd, "r"), pclose); //in here had a bug
...
void addApp(json& client) 
{
    ...
    int i = workspaceNum / COLS, j = workspaceNum % COLS; //here had a bug
    ...
    //numOfApps[int(newApp["workspace"]["id"]) - 1] += int(newApp["countAs"]); //in here had a bug
...
void scaleWindows() 
{
    ...
    {
        if (workspaces[i].size() == 0) //here too
        {
