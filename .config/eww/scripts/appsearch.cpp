...
void splitString(const std::string& str, const char delimiter, std::vector<std::string>& result) 
{
    std::string line;
...
void tryAppSearch() 
{
    ...
    for (int i = 0; i < entryNames.size(); i++) 
    {
        ...
        if (appEntries.contains(entryName)) 
        {
            cout << appEntries[entryName];
        }
        else 
        {
            cout << R"({"name":"Unknown","exec":"", "icon":""})";
        }
...
void tryCalculate() 
{
    ...
    auto eqPos = results.find('=');
    if (eqPos == string::npos) return;
    results = results.substr(eqPos + 2);
...
void commandOnly() 
{
    ...
    if (updateInfo) 
    {
        string updateCmd =
            ...
            {
                {"name", searchTerm},
                {"icon", "image/svg/dark/protocol.svg"},
                {"exec", searchTerm}
            };
